...
至此，跨线程异步执行代码的机制已经编写完毕，整体其实是非常简单的。而且也不是 Qt 专属，其实任意具备事件循环的框架，都可以使用相同逻辑实现。
...
因此，我们需要一个机制来判断依赖资源的生命周期。由于在接口层可以做各式封装，最终传递到执行点的判断方式，可通过  [std::function\<bool(void)\>](https://en.cppreference.com/w/cpp/utility/functional/function) 来表达，在执行前进行判断：
...
- 仿信号槽方式，传递 `QObject*` 指针，接口层通过 [QPointer](https://doc.qt.io/qt-5/qpointer.html) 类监测其存货状态，并将其封装为回调函数；
...
根据上文代码，此机制的接口需要提供 `(执行线程, 回调函数)` 二元组作为输入参数。
...
回调函数最终传递至内部实现的版本，便是上文所述的  [std::function\<QVariant(void)\>](https://en.cppreference.com/w/cpp/utility/functional/function) 对象。但为方便使用，我们可以提供 `Func function, Args&&... args` 形式的模板接口，用于承接任意类型的函数子和输入参数：
...
该模板类重载了逗号运算符，然后再通过特化匹配到不同版本的实现，对于有返回值的版本，将返回值储存至构造时输入的对象指针中。
...
这个等待机制，相比很多人都已经在自己的项目中广泛应用，即使用计时器配合 [QCoreApplication::processEvents()](https://doc.qt.io/qt-5/qcoreapplication.html#processEvents) 实现不阻塞事件循环的延时：
...
但由于要避免阻塞事件循环，我们不能直接调用 [std::future](https://en.cppreference.com/w/cpp/thread/future) 的对应函数，因此需要自行封装执行状态。
...
则 `wait_for()` 和 `wait_until()` 函数在完成等待后，返回 `status()` 即可；`wait()` 则是将该函数作为判断条件传给上一节的 `Wait()` 函数。

`get()` 函数同理，可以直接使用 `wait()` 完成等待，然后返回 [std::future::get()](https://en.cppreference.com/w/cpp/thread/future/get) 即可。

`valid()` 函数则是同时判断 [std::future::valid()](https://en.cppreference.com/w/cpp/thread/future/valid) 和 `executed` 状态，即将 `status() == std::future_status::ready`。
...
单元测试使用 [Qt Test](https://doc.qt.io/qt-5/qtest-overview.html) 编写，可使用 CMake 的 [CTest](https://cmake.org/cmake/help/v2.8.12/ctest.html) 机制直接执行（如 `make test`），但该执行方式下无法看到 [Qt Test](https://doc.qt.io/qt-5/qtest-overview.html) 输出。
...
库文件会生成至 `KtUtils/lib` 目录，`dll`文件(特例)和单元测试的`exe`文件会生成至 `KtUtils/bin` 目录，库文件名称为 `KtUtils`/`KtUtilsd`(Debug 后缀)。
